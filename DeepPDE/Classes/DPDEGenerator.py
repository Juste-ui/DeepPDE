...
class DPDEGenerator(keras.utils.Sequence):
    ...
    def __init__(self, batch_size:int, normalised_min:int, normalised_max:int, dimension_states:int,dimension_param:int):
    ...
    def __getitem__(self, idx):
        ...
        data_train_interior = np.random.uniform(
            self.normalised_min, self.normalised_max, [self.batch_size, self.dimension_total]) 

        t_train_initial = self.normalised_min * np.ones((self.batch_size, 1))
        
        s_and_p_train_initial = np.random.uniform(
            self.normalised_min, self.normalised_max,
